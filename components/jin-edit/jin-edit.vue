<template>
	...
	<view class="tool-view" > 
		...
		<!-- 文字相关操作 -->
		<view class="font-more"  v-if="showMoreTool" :class="showMoreTool?'showMoreTool':''" >
		...
		<editor
			:class="showMoreTool?'havetool':'notool'"
			class="ql-container"
...
export default {
	...
	methods: {
		...
		showMore() {
			...
			//this.showTool = false; this.showMoreTool=false;
			//console.log(this.showTool,this.showMoreTool);
			//this.editorCtx.setContents()
		},
		setBold() {
		...
		async getContent(){
			let content = await new Promise(resole=>{
